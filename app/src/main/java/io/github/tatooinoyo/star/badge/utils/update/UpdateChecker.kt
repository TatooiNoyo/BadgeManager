...
/**
 * 更新检查：先打 Cloudflare Worker（短超时），失败再 GitHub Releases。
 */
class UpdateChecker(
    context: Context,
    ...
    suspend fun check(force: Boolean = false): UpdateCheckResult = withContext(Dispatchers.IO) {
        ...
        for (channel in listOf(Channel.WORKER, Channel.GITHUB)) {
            val result = runChannel(channel, localVersion, localCode)
            when {
                result.isSuccess -> {
                    val info = result.getOrThrow()
                    val checkResult = toCheckResult(info, localVersion, localCode)
    ...
    private fun toCheckResult(
        ...
        return if (newerByName || newerByCode) {
            ...
            UpdateCheckResult.UpToDate(localVersion, info.versionName)
        }
    }
    ...
    private fun readCacheIfFresh(): UpdateCheckResult? {
        ...
        return when (status) {
            ...
            STATUS_UP_TO_DATE -> UpdateCheckResult.UpToDate(BuildConfig.VERSION_NAME, latest)
            else -> null
        }
    }
